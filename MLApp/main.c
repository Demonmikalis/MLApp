...
#include "CLib/MinHeap.h"
#include "CLib/HashMap.h"
#include "CLib/HashSet.h"
#include "CLib/HMem.h"
#include "CLib/HMath.h"
#include "CLib/MathInterf.h"
#include "CLib/Stack.h"
#include "Expression.h"
#include "GoBangAI.h"
#include "RegTrees/DecisionTree.h"
...
void test_HMath()
{
	...
	// matrix dot vector
	DVector v3 = CreateDVector(&gstack,3);
    DVector v4 = CreateDVector(&gstack,3);
	for(i=1;i<=3;i++) v3[i]=i;
    Dmat_dot_vec(m3,v3,v4);
    
	ShowDVector("v4",v4,3);
    FreeDVector(&gstack,v4);
	FreeDVector(&gstack,v3);
    // release
...
{
    char exp[] = "(3*4)^2;";
    double ans =  EvaluateExpression(exp);
...
void test_decisionTree()
{
    ...
    ClassificationStruct *cs = (ClassificationStruct*)malloc(sizeof(ClassificationStruct));
    readTheProblem(filePath,cs);
    printf("%f\n",cs->features[89][0]);
    ...
    double x[5] = {1,1,2.5,3.6,3.6};
    int hash_sz = 0;
    double * ans = createUniqueValues(x,5,&hash_sz);
    for(j=0;j<hash_sz;j++)
    {
        printf("%f ",ans[j]);
    }
